        ...
        {showDropdown && (
          ...
          >
            <div className="flex gap-2 p-3 bg-blue-50/30 overflow-x-auto scrollbar-hide border-b border-white/20">
              ...
              <QuickAction icon={<Home size={14} />} label="Home" onClick={() => {
                 const point = { lat: 13.0827, lng: 80.2707, name: 'Home (Chennai)' };
                 if (activeInput === 'start') setStart(point);
                 else setDestination(point);
                 setShowDropdown(false);
              }} />
              <QuickAction icon={<ShieldCheck size={14} />} label="Safe Hubs" onClick={() => {
                 const point = { lat: 10.9961, lng: 76.9637, name: 'Safe Hub (Coimbatore)' };
                 if (activeInput === 'start') setStart(point);
                 else setDestination(point);
                 setShowDropdown(false);
              }} />
            ...
            {!loading && query[activeInput] && !suggestions.length && (
              <div className="p-16 text-center text-slate-400 bg-slate-50/50">
                ...
                <p className="text-[10px] uppercase font-black mt-1 tracking-widest text-slate-400">Expand your search across the network</p>
              </div>
            )}
            ...
            {!query[activeInput] && (
               <div className="p-6">
                 ...
                 <div className="grid grid-cols-1 gap-2">
                   {[
                     { name: 'Chennai Central Station', sub: 'Hub - 98% Safety Score', lat: 13.0827, lng: 80.2707 },
                     { name: 'Coimbatore Junction', sub: 'Region - 95% Match', lat: 10.9961, lng: 76.9637 },
                     { name: 'Madurai Temple Center', sub: 'Point - Verified Secure', lat: 9.9195, lng: 78.1193 }
                   ].map((hp, i) => (
                     <button
                       key={i}
                       onClick={() => {
                         const point = { lat: hp.lat, lng: hp.lng, name: hp.name };
                         if (activeInput === 'start') setStart(point);
                         else setDestination(point);
                         setShowDropdown(false);
                       }}
                       className="w-full flex items-center gap-5 p-4 hover:bg-slate-50 rounded-2xl text-slate-600 transition-all text-left border border-slate-50"
                     >
